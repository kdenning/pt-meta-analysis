...
The estimate of intercept represents when pt_comparison = 0 (i.e., pt_v_control). The effect of the objective condition represented by the slope of pt_comparison:pt_v_objective. The summary effect size would be found by adding them together. The moderator test is significant.
...
And these two comparisons are significantly different from one another (in looking up how to use Metafor, it was demonstrated by the creators that it was necessary to do this):
...
```{r}
summary(glht(meta_multi2, linfct=contrMat(c("pt_v_control:Stereotyping"=1,
...
Checking if the two interaction effects differ from one another again:

```{r}
anova(meta_multi2_ptus, btt=5:6)
```

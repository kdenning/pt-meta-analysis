...
#Function to get I2
get_I2 <- function(.x, .y) { #.x is the dataset supplied to the rma.mv function, .y is the output of that function
  W <- diag(1/.x$var_dunb)
  X <- model.matrix(.y)
  P <- W - W %*% X %*% solve(t(X) %*% W %*% X) %*% t(X) %*% W
...
get_I2(meta_overall1, meta_overall)
```

Across perspective taking condition and outcome, there is a small but statistically significant effect. However, with Q statistically significant and I2 demonstrating that 95% of the total variance is due to heterogeneity, this effect is is pretty much meaningless. We need to look at smaller subsets of data.
...
# But hetereogeneity is still significant in all stereotyping outcomes...
...
Stereotyping is significantly moderated by scale. It seems we should look at which the effect of PT instruction within the subset of data for each scale to determine if heterogeneity is no longer significant. (And look into other measures of hetereogeneity)
...
```{r overlap data}
#the overall dataset was already cleaned and set-up in the correct comparisons
overlap_data <- meta_overall1 %>% 
...
## Overall Overlap model - no mods

```{r overlap model no mods}
overlap_nomods <- rma.mv(dunb, 
                      var_dunb,
                      random = list(~ 1 | sample_number_total/conditions_within_sample_var/outcomes_within_sample_var, 
...
Not significant - did I run this earlier? This estimate looks familiar
...
Intercept is marginally significant from 0, but the contrast is not significantly different from the intercept.

```{r for the individual effects}
...
### Moderator target in need variable or out-group

```{r overlap model sample size}
overlap_pt_targdescrip1 <- rma.mv(dunb, 
                      var_dunb,
                      mods = ~ pt_comparison*target_empathetic_need + 
                        pt_comparison*target_out_minor +
                        pt_comparison*target_ingroup_nonspecific +
                        pt_comparison*target_adversary +
                        pt_comparison*target_information,
                      random = list(~ 1 | sample_number_total/conditions_within_sample_var/outcomes_within_sample_var, 
                                    ~ 1 | outcome_scale_group), data = overlap_data)

overlap_pt_targdescrip
```

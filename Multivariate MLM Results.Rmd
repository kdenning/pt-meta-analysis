...
psych::describe(descrip_data) # this all looks correct
```
...
There are some outlier data points, which was also indicated by the histogram. We will want to check those that are particularly high and alone to see if they were coded incorrectly by us or in the effect size conversion.

## Checking Outliers

### Outliers over 2 or -2
...
#### Checking escalc as an individual
...
Slight variation, but pretty much the same - escalc seems to be doing things correctly
...
Also roughly the same - it appears the calculation is correct. The correlation is VERY large, so this makes sense. Need to re-check the data (for the 5th time) from the articles to see if what we have in the dataset is correct for the outlier data.
...
Overlap and stereotyping do significantly differ from one another.
...
mlma_egg_mod_sda <- rma.mv(dunb ~ 1 + sda, 
                           V = var_dunb, 
                           random = list(~ 1 | authors/sample_number_total/outcomes_within_sample_var/effect_size_num,

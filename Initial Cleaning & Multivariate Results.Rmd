...
# Get subsets of the overall dataset corresponding to each comparison
## Get labels for the filter in the for loop corresponding to each comparison
...
meta_output <- map(df_4_overall, ~rma.mv(dunb,
                         ...
                         data = .x))
# Mapping over the output of each meta-analysis to get I2 values
## Overall I2 value for model
I2_overall_output <- data.frame(map2(df_4_overall, meta_output, ~get_I2_overall(.x, .y))) %>% 
  pivot_longer(1:6) %>% 
...
### Getting values for bewteen study into dataframe column to put in table later
between_study_I2 <- list(length = length(I2_var_levels))

for (i in seq_along(I2_var_levels)){
 between_study_I2[[i]] <- I2_var_levels[[i]][1]
}
...
### Getting values for between outcome into dataframe column to put in table later
within_study_I2 <- list(length = length(I2_var_levels))

for (i in seq_along(I2_var_levels)){
 within_study_I2[[i]] <- I2_var_levels[[i]][2]
}

within_I2 <- data.frame(within_study_I2) %>% 
  pivot_longer(1:6) %>% 
  mutate(I2_within = value) %>% 
  select(-value, -name)

### Getting values for crossed scale into dataframe column to put in table later

cross_measure_I2 <- list(length = length(I2_var_levels))

for (i in seq_along(I2_var_levels)){
 cross_measure_I2[[i]] <- I2_var_levels[[i]][2]
}

cross_I2 <- data.frame(cross_measure_I2) %>% 
  pivot_longer(1:6) %>% 
  mutate(I2_crossed = value) %>% 
  select(-value, -name)

# Mapping over each model to pull out the data so we can put it into one dataframe
estimates <- map(meta_output, ~coef(.x)) %>% 
  as.data.frame() %>% 
  pivot_longer(1:6) %>% 
...
pval <- map(meta_output, ~get_pval_rma(.x)) %>% 
  as.data.frame() %>% 
  pivot_longer(1:6) %>% 
...
ci_upper <- map(meta_output, ~get_ci_ub_rma(.x)) %>% 
  as.data.frame() %>% 
  pivot_longer(1:6) %>% 
...
ci_lower <- map(meta_output, ~get_ci_lb_rma(.x)) %>% 
  as.data.frame() %>% 
  pivot_longer(1:6) %>% 
  mutate(ci_lower = value) %>% 
  dplyr::select(-name, - value) %>% 
  round(digits = 3)

Q_val <- map(meta_output, ~get_Q_rma(.x)) %>% 
  as.data.frame() %>% 
  pivot_longer(1:6) %>% 
  mutate(Q = value) %>% 
  dplyr::select(-name, - value) %>% 
...
Q_pval <- map(meta_output, ~get_Q_p_rma(.x)) %>% 
  as.data.frame() %>% 
  pivot_longer(1:6) %>% 
  mutate(Q_pval = value) %>% 
  dplyr::select(-name, - value) %>% 

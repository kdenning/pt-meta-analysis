...
---
```{r target outgroup moderator stereo 1, echo = FALSE, warning = FALSE, message = FALSE, error = FALSE}
### Trying to get a function to work for target moderators
...
df_4_stereo1_mod_wide <- list(length = length(stereo_pt_filter1))

## The for loop getting us subsetted datasets
for (i in seq_along(stereo_pt_filter1)){ # change () in seq_along to filters
  df_4_stereo1[[i]] <- meta_stereo1_mod_wide %>%  # change data that is subsetted [[i]] to the empty list above & change the piped data to the correct cleaned dataset for this analysis
...
df_4_stereo1_mod_wide <- list(length = length(4))

 #x is the list of moderator variables
for (i in seq_along(meta_stereo1_mod)){ #x is the dataset for the meta-analysis, y is the moderators you want to cycle through as a long format variable
  ...
  df_4_stereo1_mod_wide[[i]] <- stereo_meta1_targinfo
}
...
prac <- for (y in seq_along(meta_stereo1_mod$target_mod)){
                        return(meta_stereo1_mod$target_mod[y])
                        }
...
stereo_meta1_targinfo <- rma.mv(dunb, 
                      ...
                      mods = ~ ,
                      random = list(~ 1 | sample_number_total/conditions_within_sample_var/outcomes_within_sample_var, 
                                    ~ 1 | outcome_scale_group), data = meta_stereo1_mod)
...
levels(meta_stereo1_mod$target_mod)
```
